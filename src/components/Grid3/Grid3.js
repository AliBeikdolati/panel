...
export default Component => {
    const wrapper = class RTFixedDraggableColumn extends React.Component {
        ...
        constructor(props) {
            ...
            this.columnReorderedByHOC = false; // track if column reorder happened in or outside of ReactTable
        }
        ...
        findParentHeader(element) {
            ...
            if (
                ...
                element.className.includes('rt-th')
            ) {
                return element;
            } else {
            ...
            headers.forEach((header, i) => {
                ...
                if (header.className.includes('enable-drag')) {
                    ...
                    headerParent.ondrop = e => {
                        ...
                        this.reorderIndicatorUp.style.display = 'none';
                        this.reorderIndicatorDown.style.display = 'none';
                        this.columnReorderedByHOC = true;
                        // trigger a re-render
                    ...
                    if (enableColumnWideDrag) {
                        headerParent.setAttribute(draggableAttr, false);
                    } else {
        ...
        componentDidUpdate(prevProps) {
            ...
            if (!this.state.firstLoad && this.columnReorderedByHOC) {
                ...
                const originalOrder = prevProps.columns.map(col => {
                    if (typeof col.accessor === 'function') return col.id;
                    return col.accessor;
                });

                const newOrder = this.props.columns.map(col => {
                    if (typeof col.accessor === 'function') return col.id;
                    return col.accessor;
                });

                if (this.reorder.length > 0 && JSON.stringify(originalOrder) !== JSON.stringify(newOrder)) {
                    this.reorder = [];
        ...
        render() {
            ...
            if (mode && mode === DragMode.SWAP) {
                ...
                this.reorder.forEach(
                    o =>
                        (this.colsNoExtraContainingDiv[o.a] = this.colsNoExtraContainingDiv.splice(
                            o.b,
                            1,
                            this.colsNoExtraContainingDiv[o.a]
                        )[0])
                );
            } else {
                ...
                this.reorder.forEach(o =>
                    this.colsNoExtraContainingDiv.splice(
                        o.a,
                        0,
                        this.colsNoExtraContainingDiv.splice(o.b, 1)[0]
                    )
                );
            }

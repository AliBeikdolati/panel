...
function render(self) {
    let container = isDOMElement(self.node) ? self.node : document.getElementById(self.node);
    let leaves = [], click;
    let renderLeaf = function (item) {
        let leaf = document.createElement('div');
        let content = document.createElement('div');
        let text = document.createElement('div');
        let expando = document.createElement('i');
        ...
        expando.setAttribute('class', 'tree-expando material-icons' + (item.expanded ? 'expanded' : ''));
        expando.textContent = item.expanded ? 'chevron_left' : 'expand_more';
    ...
    click = function (e) {
        let parent = (e.target || e.currentTarget);
        if (!e.currentTarget.classList.contains('tree-leaf-content')) {
...
TreeView.prototype.expand = function (node, leaves, skipEmit) {
    ...
    expando.textContent = 'chevron_left';
...
TreeView.prototype.collapse = function (node, leaves, skipEmit) {
    let expando = node.querySelector('.tree-expando');
    expando.textContent = 'expand_more';
